...
// 初始化
function init() {
  console.log('[Variational Sync] Content script loaded');

  // 从 storage 恢复按钮选择器
  chrome.storage.local.get(['variational_button_selector'], (result) => {
    if (result.variational_button_selector) {
      buttonSelector = result.variational_button_selector;
      console.log('[Variational Sync] Restored button selector:', buttonSelector);

      // 尝试找到按钮
      try {
        selectedButton = document.querySelector(buttonSelector);
        if (selectedButton) {
          console.log('[Variational Sync] ✓ Button found using saved selector');
        }
      } catch (e) {
        console.warn('[Variational Sync] Could not find button with selector:', buttonSelector);
      }
    }
  });
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    console.log('[Variational Sync] Received message:', message);
...
function handleButtonSelection(e) {
  ...
  console.log('[Variational Sync] Button selected:', selectedButton);
  console.log('[Variational Sync] Selector:', buttonSelector);
  ...
  chrome.storage.local.set({
    variational_button_saved: true,
    variational_button_selector: buttonSelector
  });

  alert('✅ Variational 提交按钮已保存！\n请在 Variational 页面也选择一次提交按钮。');
}
...
function updateFloatingButtonStatus() {
  ...
  if (selectedButton || buttonSelector) {
    textEl.textContent = '✅ Variational 已就绪';
...
async function performSyncClick(syncTimestamp) {
  console.log('[Variational Sync] performSyncClick called');
  console.log('[Variational Sync] Current time:', Date.now());
  console.log('[Variational Sync] Sync timestamp:', syncTimestamp);
  ...
  if (!button && buttonSelector) {
    console.log('[Variational Sync] Button reference lost, trying to find using selector...');
    try {
      button = document.querySelector(buttonSelector);
      if (button) {
        selectedButton = button;
        console.log('[Variational Sync] ✓ Button found using selector');
      }
    } catch (e) {
      console.error('[Variational Sync] Error finding button:', e);
    }
  }

  if (!button) {
    const error = 'No button selected or button not found!';
    console.error('[Variational Sync]', error);
  ...
  console.log(`[Variational Sync] Scheduled click in ${delay}ms`);
    ...
    if (delay > 0) {
      setTimeout(() => {
        try {
          ...
          console.log(`[Variational Sync] Clicking now!`);
          console.log(`[Variational Sync] Actual time: ${actualTime}`);
          console.log(`[Variational Sync] Time diff: ${timeDiff}ms`);

          button.click();
          showClickFeedback('✅ Variational 已点击！');
          resolve();
        } catch (error) {
          console.error('[Variational Sync] Click error:', error);
          showClickFeedback('❌ 点击失败', 'error');
          reject(error);
        }
      }, delay);
    } else {
      console.log(`[Variational Sync] Timestamp已过，立即点击`);
      try {
        button.click();
        showClickFeedback('✅ Variational 已点击！');
        resolve();
